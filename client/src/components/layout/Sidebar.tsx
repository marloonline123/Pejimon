              ...
              >
                <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-rich-cerulean-500/20 text-rich-cerulean-600 dark:text-rich-cerulean-400 overflow-hidden">
                  {activeOrganization?.logo ? (
                    <img src={activeOrganization.logo} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-[10px] font-bold uppercase">
                      {activeOrganization ? activeOrganization.name.substring(0, 2) : "OR"}
                    </span>
                  )}
                </div>
                <div
                ...
                {!isCollapsed && <ChevronDown className="w-3 h-3 ml-auto text-taupe-500 shrink-0" />}
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-56">
                {organizations?.map((org: any) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      "cursor-pointer",
                      activeOrganization?.id === org.id && "bg-rich-cerulean-500/10 text-rich-cerulean-600 font-medium"
                    )}
                  >
                    <div className="flex items-center gap-2 w-full">
                      <div className="w-6 h-6 rounded-md bg-alice-blue-800 dark:bg-ink-black-300 flex items-center justify-center shrink-0 overflow-hidden">
                        {org.logo ? (
                          ...
                          <span className="text-[10px] font-bold uppercase">{org.name.substring(0, 2)}</span>
                        )}
                      </div>
                ...
                <DropdownMenuItem>
                  <Link href="/select-organization" className="w-full cursor-pointer flex items-center text-taupe-500 dark:text-taupe-400">
